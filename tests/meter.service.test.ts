...
beforeAll(async () => {
  await prisma.planLimit.upsert({
    where: { id: "free" },
    update: { apiCallLimit: 1000, aiTokenLimit: BigInt(100000) },
    create: { id: "free", apiCallLimit: 1000, aiTokenLimit: BigInt(100000) },
  });
  const tenant = await prisma.tenant.create({
    data: { name: "Test Tenant", apiKey: `test-${Date.now()}`, plan: "free" },
  });
  tenantId = tenant.id;
});
...
describe("meterService.record", () => {
  ...
  it("prices ai_tokens with reasoning billed as output", async () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.costCents).toBeGreaterThan(0);
  });
});